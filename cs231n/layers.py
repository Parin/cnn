...
def affine_forward(x, w, b):
  ...
  N = x.shape[0]
  D = np.prod(x.shape[1:]) # D = d1 * d2 * d3 ... dk
  
  x_reshaped = x.reshape(N, D)
  out = x_reshaped.dot(w) + b
  cache = (x, w, b, x_reshaped, out)
  return out, cache
...
def affine_backward(dout, cache):
  ...
  """
  x, w, b, x_reshaped, out = cache
  dx, dw, db = None, None, None
  #############################################################################
  # Implement the affine backward pass.                                 #
  #############################################################################
  
  N, D = x_reshaped.shape
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x, sample_mean, sample_var, norm_x, gamma, beta, eps)
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  N, D = dout.shape
  x, sample_mean, sample_var, norm_x, gamma, beta, eps = cache
  
  dnorm_x = dout * gamma
  
  q1 = dnorm_x
  q2 = x - sample_mean
  q3 = (-1.0 / 2.0) * (sample_var + eps) ** (-3.0 / 2.0)
  dsample_var = np.sum(q1 * q2 * q3, axis=0)
  
  q1 = np.sum(dnorm_x * (-1.0 / np.sqrt(sample_var + eps)), axis=0)
  q2 = dsample_var * (1.0 / N) * (np.sum(-2.0 * (x - sample_mean), axis=0))
  dsample_mean = q1 + q2
  
  q1 = dnorm_x * (1.0 / np.sqrt(sample_var + eps))
  q2 = dsample_var * ((2.0 / N) * (x - sample_mean))
  q3 = dsample_mean / N
  dx = q1 + q2 + q3
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  # im2col on x
  x_col = im2col_indices(x, HH, WW, padding=padding, stride=stride)
  w_row = np.reshape(w, (F, -1))
  res = np.dot(w_row, x_col) + b.reshape(-1, 1)
  
  # reshape output
  out = res.reshape(F, H_out, W_out, N)
...
def conv_backward_naive(dout, cache):
  ...
  da_dx = w_row
  da_dw = x_col
  dL_da = np.reshape(dout, (H_out * W_out * N, F))
  
  dw = np.dot(da_dw, dL_da)
  print dw.shape
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  x, sample_mean, sample_var, norm_x, gamma, beta, eps = cache
  x = x.reshape(N, H, W, C).transpose(0, 3, 1, 2)
  norm_x = norm_x.reshape(N, H, W, C).transpose(0, 3, 1, 2)
  cache = (x, sample_mean, sample_var, norm_x, gamma, beta, eps)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  norm_x = norm_x.transpose(0, 2, 3, 1).reshape(N * H * W, C)
  cache = (x, sample_mean, sample_var, norm_x, gamma, beta, eps)
