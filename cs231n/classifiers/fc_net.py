...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    h1, cache2 = relu_forward(z1)
    scores, cache3 = affine_forward(h1, self.params['W2'], self.params['b2'])
    ...
    dh1, dw2, db2 = affine_backward(dscores, cache3)
    dz1 = relu_backward(dh1, cache2)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    dims_out = hidden_dims + [num_classes]
    dims_layer = zip(dims_in, dims_out)
    
    # using random initialization
    for layer in xrange(0, self.num_layers):
        self.params['W' + str(layer + 1)] = weight_scale * np.random.randn(dims_layer[layer][0], dims_layer[layer][1])
        self.params['b' + str(layer + 1)] = np.zeros(dims_layer[layer][1])

    # using Xavier initialization
    #for layer in xrange(0, self.num_layers):
    #    self.params['W' + str(layer + 1)] = (np.random.randn(dims_layer[layer][0], dims_layer[layer][1]))/np.sqrt(dims_layer[layer][0]/2)
    #    self.params['b' + str(layer + 1)] = np.zeros(dims_layer[layer][1])
    ...
    if self.use_batchnorm:
      for layer in xrange(0, self.num_layers - 1):
          self.params['gamma' + str(layer + 1)] = np.ones(dims_layer[layer][1])
          self.params['beta' + str(layer + 1)] = np.zeros(dims_layer[layer][1])
      self.bn_params = [{'mode': 'train'} for i in xrange(self.num_layers - 1)]
    
    # Cast all parameters to the correct datatype
    for k, v in self.params.iteritems():
        #if not k.startswith('gamma') and not k.startswith('beta'):
  ...
  def loss(self, X, y=None):
    ...
    # store caches of forward pass
    fp_caches = [] 
    
    # initialize a to input data used in the loop
    a = X
    
    if self.use_batchnorm and self.use_dropout:
        
        for layer in xrange(0, self.num_layers - 1):
            a, cache = affine_bn_relu_do_forward(
                a, 
            ...
            fp_caches.append(cache)
            
    elif self.use_batchnorm and not self.use_dropout:
        
        for layer in xrange(0, self.num_layers - 1):
            a, cache = affine_bn_relu_forward(
                a, 
                self.params['W' + str(layer + 1)], 
                self.params['b' + str(layer + 1)],
                self.params['gamma' + str(layer + 1)],
        ...
        for layer in xrange(0, self.num_layers - 1):
            a, cache = affine_relu_do_forward(
                a, 
                self.params['W' + str(layer + 1)], 
                self.params['b' + str(layer + 1)],
                self.dropout_param
    ...
    if self.use_batchnorm and self.use_dropout:
        ...
        for layer in xrange(self.num_layers - 1, 0, -1):
            dx, dw, db, dgamma, dbeta = affine_bn_relu_do_backward(dx, fp_caches[layer - 1])
        ...
        for layer in xrange(self.num_layers - 1, 0, -1):
            dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx, fp_caches[layer - 1])
            grads['W' + str(layer)] = dw
            grads['b' + str(layer)] = db
            grads['gamma' + str(layer)] = dgamma
            grads['beta' + str(layer)] = dbeta
        ...
        for layer in xrange(self.num_layers - 1, 0, -1):
            dx, dw, db = affine_relu_do_backward(dx, fp_caches[layer - 1])
            grads['W' + str(layer)] = dw
            grads['b' + str(layer)] = db
        ...
        for layer in xrange(self.num_layers - 1, 0, -1):
            dx, dw, db = affine_relu_backward(dx, fp_caches[layer - 1])
            grads['W' + str(layer)] = dw
            grads['b' + str(layer)] = db
