...
def adam(x, dx, config=None):
  ...
  m_bias_corrected = config['m']/(1 - (config['beta1'] ** config['t']))
  v_bias_corrected = config['v']/(1 - (config['beta2'] ** config['t']))
  next_x = x + (-(config['learning_rate'] * m_bias_corrected / (np.sqrt(v_bias_corrected) + config['epsilon'])))
  return next_x, config
